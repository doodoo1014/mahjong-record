...
import { useState, useRef, useEffect, useMemo } from 'react';
import { Gamepad2, Plus, List, BarChart2, Trophy, ChevronLeft, Check, Trash2, ShieldAlert, Users, X, Flag, Edit, Lock, Unlock, Search, CalendarPlus, Shield, UserCheck, ShieldClose, UserX, MessageSquare, AlertOctagon, PieChart, BarChart, Bell, ArrowUpDown, Swords, Info, UserPlus, Database } from 'lucide-react';
import { db } from './firebase'; 
import { collection, doc, setDoc, getDoc, getDocs, updateDoc, deleteDoc, onSnapshot, query, orderBy, where } from 'firebase/firestore'; 
...
const hasYakuman = (game) => {
  if (!game || !game.rounds || !Array.isArray(game.rounds)) return false;
  return game.rounds.filter(r => r.isActive !== false).some(r => {
    if (r?.type !== '화료') return false;
    if (Number(r?.han || 0) >= 13) return true;
    const selected = r?.selectedYaku || [];
    if (!Array.isArray(selected)) return false;
    return selected.some(y => (yakuData['역만']?.includes(y)) || (yakuData['더블역만']?.includes(y)));
  });
};
...
function App() {
  ...
  const [newPlayerBaseName, setNewPlayerBaseName] = useState('');

  // 커스텀 자동완성(Autocomplete)용 포커스 상태
  ...
  useEffect(() => {
    ...
    return () => { unsubGames(); unsubSeasons(); unsubPlayers(); };
  }, []);
  ...
  // 💡 [핵심] 기존 데이터 마이그레이션 함수 (마스터 대시보드용)
  const handleMigrateDB = async () => {
    if (!confirm("⚠️ 주의: 기존 대국 기록의 모든 플레이어 이름을 '이름A' 형태로 변환하고 players 컬렉션에 등록합니다.\n\n데이터베이스 구조를 한 번만 변경하는 작업입니다. 진행하시겠습니까?")) return;

    try {
      const gamesSnap = await getDocs(collection(db, 'games'));
      const allGamesData = gamesSnap.docs.map(d => ({ id: d.id, ...d.data() }));

      // 1. 모든 고유 플레이어 이름 추출
      const uniqueNames = new Set();
      allGamesData.forEach(g => {
        if (g.players) g.players.forEach(p => uniqueNames.add(p));
      });

      const playerMap = {}; 

      // 2. players 컬렉션에 등록 및 A 붙이기
      for (const oldName of uniqueNames) {
        let newName = oldName;
        let baseName = oldName;
        let alphabet = 'A';

        // 이미 끝에 대문자 알파벳이 있는지 검사 (없으면 A를 붙임)
        if (!/[A-Z]$/.test(oldName)) {
          newName = oldName + 'A';
          baseName = oldName;
          playerMap[oldName] = newName;
          
          await setDoc(doc(db, 'players', newName), {
            baseName: baseName, alphabet: alphabet, fullName: newName, isActive: true, registeredAt: Date.now()
          });
        } else {
           baseName = oldName.slice(0, -1);
           alphabet = oldName.slice(-1);
           playerMap[oldName] = oldName; // 변경 없음

           await setDoc(doc(db, 'players', oldName), {
            baseName: baseName, alphabet: alphabet, fullName: oldName, isActive: true, registeredAt: Date.now()
          });
        }
      }

      // 3. games 문서의 이름들 업데이트
      for (const g of allGamesData) {
        let isUpdated = false;
        
        const newPlayers = g.players.map(p => {
          if (playerMap[p] && playerMap[p] !== p) { isUpdated = true; return playerMap[p]; }
          return p;
        });

        const newRounds = (g.rounds || []).map(r => {
          let newR = { ...r };
          if (r.winner && playerMap[r.winner] && playerMap[r.winner] !== r.winner) { newR.winner = playerMap[r.winner]; isUpdated = true; }
          if (r.loser && playerMap[r.loser] && playerMap[r.loser] !== r.loser) { newR.loser = playerMap[r.loser]; isUpdated = true; }
          if (r.chomboPlayer && playerMap[r.chomboPlayer] && playerMap[r.chomboPlayer] !== r.chomboPlayer) { newR.chomboPlayer = playerMap[r.chomboPlayer]; isUpdated = true; }
          if (r.tenpaiPlayers) { newR.tenpaiPlayers = r.tenpaiPlayers.map(p => { if(playerMap[p] && playerMap[p] !== p){ isUpdated=true; return playerMap[p]; } return p; }); }
          if (r.nagashiMangan) { newR.nagashiMangan = r.nagashiMangan.map(p => { if(playerMap[p] && playerMap[p] !== p){ isUpdated=true; return playerMap[p]; } return p; }); }
          return newR;
        });

        if (isUpdated) {
          await updateDoc(doc(db, 'games', g.id), { players: newPlayers, rounds: newRounds });
        }
      }
      alert("✅ 데이터베이스 마이그레이션이 완료되었습니다!");
    } catch (e) {
      console.error(e);
  ...
  const handleRegisterPlayer = async () => {
    if (!newPlayerBaseName.trim() || /[a-zA-Z]/.test(newPlayerBaseName)) {
      return alert('알파벳을 제외한 한글/숫자 이름만 기입해주세요.');
    }
    const q = query(collection(db, 'players'), where('baseName', '==', newPlayerBaseName));
    const snapshot = await getDocs(q);
    const alphabet = String.fromCharCode(65 + snapshot.size);
    const fullName = `${newPlayerBaseName}${alphabet}`;

    await setDoc(doc(db, 'players', fullName), {
      baseName: newPlayerBaseName, alphabet: alphabet, fullName: fullName, isActive: true, registeredAt: Date.now()
    });

    alert(`${fullName} 등록 완료! (화면에는 '${newPlayerBaseName}'으로 표시됩니다)`);
    setIsNewPlayerModalOpen(false);
    setNewPlayerBaseName('');
  };
  ...
  const displayedGames = (games || []).filter(g => {
    ...
    if (searchQuery) {
      const matchDate = g.date?.includes(searchQuery);
      const matchPlayer = g.players?.some(p => displayName(p).includes(searchQuery));
      if (!matchDate && !matchPlayer) return false;
    }

    if (gameFilter === '역만') return hasYakuman(g);
    if (gameFilter === '촌보') return g.rounds?.filter(r => r.isActive !== false).some(r => r.type === '촌보') ?? false;
    if (gameFilter === '코멘트') return g.rounds?.filter(r => r.isActive !== false).some(r => r.comment && r.comment.trim() !== '') ?? false;
    return true;
  });
  ...
  const currentGame = games.find(g => g.id === selectedGameId);
  const records = currentGame ? currentGame.rounds.filter(r => r.isActive !== false) : [];
  const players = currentGame ? currentGame.players : [];
  ...
  const handleCreateNewGame = async () => {
    ...
    await setDoc(doc(db, 'games', newGame.id.toString()), newGame);
    setIsNewGameModalOpen(false); setSelectedGameId(newGame.id); setActiveNav('기록'); 
    setPlayerE(''); setPlayerS(''); setPlayerW(''); setPlayerN(''); setNewGameDate(getTodayStr());
  };
  ...
  const handleOpenNewRound = () => {
    ...
    const validRounds = currentGame && currentGame.rounds ? currentGame.rounds.filter(r => r.isActive !== false) : [];
    if (validRounds.length > 0) {
      const lastRound = validRounds[0]; 
      const maxRound = currentGame.type === '3인' ? 3 : 4;
      ...
      if (lastRound.type === '촌보') {
        defaultWind = lastRound.wind; defaultRoundNum = lastRound.roundNum; defaultHonba = lastRound.honba;
      } else if (lastRound.type === '화료') {
        if (lastRound.winner === dealerName) {
          defaultHonba = lastRound.honba + 1;
        } else {
          defaultHonba = 0; defaultRoundNum = lastRound.roundNum + 1;
        }
      } else if (lastRound.type === '유국') {
        if (lastRound.abortiveType) {
          defaultHonba = lastRound.honba + 1;
          ...
          defaultHonba = lastRound.honba + 1; defaultRoundNum = lastRound.roundNum + 1;
        }
      }
  ...
  const handleEditRound = (record) => {
    ...
    if (record.type === '화료') {
      ...
      setWaitType(record.waitType || '양면'); 
      setMenzen(record.menzen || '멘젠');
      setDora(record.dora || 0); setAka(record.aka || 0); setUra(record.ura || 0); setPei(record.pei || 0);
      setFu(record.fu || 30); setHan(record.han || 1); setScore(record.score || '');
  ...
  const handleSaveRound = async () => {
    let newRound = { id: editingRoundId || Date.now(), wind, roundNum, honba, kyotaku, type: recordMode, comment: roundComment, isActive: true };
  ...
  const handleDeleteRound = async (roundId) => { 
    if(confirm("이 국의 기록을 삭제하시겠습니까?")) {
      const updatedRounds = currentGame.rounds.map(r => r.id === roundId ? { ...r, isActive: false } : r);
      await setDoc(doc(db, 'games', selectedGameId.toString()), { ...currentGame, rounds: updatedRounds });
    }
  };
  ...
  const handleDeleteGame = async (e, gameId) => { 
    e.stopPropagation(); 
    if(confirm("이 대국의 전체 기록을 삭제하시겠습니까? (DB에는 남지만 통계에서 제외됩니다)")) { 
      await updateDoc(doc(db, 'games', gameId.toString()), { isActive: false });
  ...
  const generatePlayerStats = (targetGames) => {
    ...
    targetGames.forEach(game => {
      ...
      game.rounds.filter(r => r.isActive !== false).forEach(round => {
        game.players.forEach(p => { if (stats[p]) stats[p].roundsPlayed += 1; });
        if(round.type === '화료') {
          const winnerStat = stats[round.winner];
  ...
  const selectedStatPlayer = selectedStatPlayerName ? { name: selectedStatPlayerName } : null;
  const filteredPlayerStatsList = playerStatsList.filter(p => displayName(p.name).includes(searchQuery));
  ...
  const globalYakuStats = useMemo(() => {
    const yCounts = {};
    statsGames.forEach(g => g.rounds.filter(r => r.isActive !== false).forEach(r => { if(r.type === '화료') r.selectedYaku?.forEach(y => yCounts[y] = (yCounts[y] || 0) + 1); }));
    
    const sorted = Object.entries(yCounts).map(([yaku, count]) => ({ yaku, count })).sort((a, b) => {
      if (b.count !== a.count) return b.count - a.count;
      return a.yaku.localeCompare(b.yaku);
    });
  ...
  const globalWinStats = useMemo(() => {
    const w = { menzenTsumo:0, menzenRon:0, furoTsumo:0, furoRon:0, riichi:0, dama:0, furoWin:0, wait: {}, winCount:0 };
    statsGames.forEach(g => g.rounds.filter(r => r.isActive !== false).forEach(r => {
      if(r.type === '화료') {
        w.winCount++;
        const isMenzen = r.menzen === '멘젠';
        if (r.winType === '쯔모') isMenzen ? w.menzenTsumo++ : w.furoTsumo++;
  ...
  // 💡 [핵심] 커스텀 자동완성 입력 컴포넌트 렌더링 함수
  const renderPlayerInput = (windStr, windKey, value, setter) => {
    const showDropdown = focusedInput === windKey && value.length > 0;
    // 입력한 글자(어절)가 포함된 활성 유저 필터링
    const filtered = allPlayers.filter(p => p.isActive && p.fullName.includes(value));

    return (
      <div className="relative">
        <div className="flex items-center gap-3 bg-white p-2 rounded-xl shadow-sm border border-gray-100">
          <div className="bg-[#2E7D32] text-white w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm shadow-inner">{windStr}</div>
          <input 
            type="text" 
            value={value} 
            onChange={e => setter(e.target.value)} 
            onFocus={() => setFocusedInput(windKey)}
            onBlur={() => setTimeout(() => setFocusedInput(null), 150)} // 클릭 인식을 위해 blur 지연
            placeholder={`${windStr}가 이름 (검색)`} 
            className="flex-1 text-sm font-bold bg-transparent focus:outline-none" 
          />
        </div>
        {showDropdown && filtered.length > 0 && (
          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-[#2E7D32] rounded-xl shadow-xl z-50 max-h-40 overflow-y-auto">
            {filtered.map(p => (
              <div 
                key={p.fullName} 
                onClick={() => { setter(p.fullName); setFocusedInput(null); }}
                className="p-3 border-b border-gray-100 last:border-0 hover:bg-green-50 cursor-pointer text-sm font-bold text-gray-800 flex justify-between items-center"
              >
                <span>{displayName(p.fullName)}</span>
                <span className="text-[10px] text-gray-400 font-normal">{p.fullName}</span>
              </div>
            ))}
          </div>
        )}
    ...
    <div className="flex flex-col h-screen bg-[#F5F5DC] font-sans relative overflow-hidden text-[#1A1A1A]">
      ...
      {activeNav === '기록' && selectedGameId === null && (
        <div className="flex bg-white border-b border-gray-200 shadow-sm z-10 shrink-0">
          {['전체', '4인', '3인'].map(t => (
            <button key={t} onClick={() => setActiveTab(t)} className={`flex-1 py-3 text-center font-bold flex justify-center items-center gap-1.5 border-b-2 ${activeTab === t ? 'border-[#2E7D32] text-[#2E7D32]' : 'border-transparent text-gray-500'}`}>
              {t} 게임 <span className={`text-[10px] px-2 py-0.5 rounded-full text-white ${activeTab === t ? 'bg-[#2E7D32]' : 'bg-gray-300'}`}>{t === '전체' ? displayedGames.length : displayedGames.filter(g=>g.type===t).length}</span>
            </button>
          ))}
        </div>
      )}
        ...
        {activeNav === '기록' && selectedGameId === null && (
          <div className="flex-1 flex flex-col p-4 space-y-4">
            {displayedGames.length === 0 ? (
              ...
              displayedGames.map(game => {
                const isGameYakuman = hasYakuman(game);
                const hasChomboGame = game.rounds.filter(r => r.isActive !== false).some(r => r.type === '촌보');
                
                const playerYakuman = (p) => game.rounds.filter(r => r.isActive !== false).some(r => r.type === '화료' && r.winner === p && (r.han >= 13 || r.selectedYaku?.some(y => yakuData['역만']?.includes(y) || yakuData['더블역만']?.includes(y))));
                const playerChombo = (p) => game.rounds.filter(r => r.isActive !== false).some(r => r.type === '촌보' && r.chomboPlayer === p);
                ...
                return (
                  <div key={game.id} onClick={() => setSelectedGameId(game.id)} className={`p-4 rounded-[20px] shadow-sm relative cursor-pointer hover:shadow-md transition-shadow active:scale-[0.98] border ${cardClass}`}>
                    ...
                    <div className="flex justify-between items-center mb-1">
                        <span className="text-xs font-medium text-gray-400">{game.date}</span>
                        <div className="flex items-center gap-1.5"><span className={`text-[10px] font-bold px-2 py-1 rounded ${game.status === '종료' ? 'text-gray-600 bg-gray-100' : 'text-[#2E7D32] bg-green-50'}`}>{game.rounds.filter(r => r.isActive !== false).length}국</span><span className={`text-[10px] font-bold px-2 py-1 rounded ${game.status === '종료' ? 'text-gray-500 bg-gray-200' : 'text-[#2E7D32] bg-green-50'}`}>{game.status}</span></div>
                    </div>
                    <div className="flex justify-between items-center mb-3">
                      <h3 className="font-bold text-base text-gray-800 truncate pr-4 tracking-tight">{game.players.map(displayName).join(' · ')}</h3>
                      {canWrite && <button onClick={(e) => handleDeleteGame(e, game.id)} className="text-red-300 hover:text-red-500 p-1"><Trash2 size={16} /></button>}
                    </div>
                    ...
                    {game.players.map((p, i) => {
                      ...
                      let boxBg = 'bg-white';
                      let boxBorder = 'border border-gray-100';
                      let windBg = 'bg-[#2E7D32]'; 
                      let nameColor = 'text-gray-800';

                      if (hasY && hasC) { boxBg = 'bg-white'; boxBorder = 'border-2 border-gray-600'; windBg = 'bg-red-600'; nameColor = 'text-gray-800'; }
                      else if (hasC) { boxBg = 'bg-white'; boxBorder = 'border-2 border-gray-600'; windBg = 'bg-gray-600'; nameColor = 'text-gray-800'; }
                      else if (hasY) { boxBg = 'bg-white'; boxBorder = 'border-2 border-red-500'; windBg = 'bg-red-600'; nameColor = 'text-gray-800'; }

                      if (isTobi) { boxBg = 'bg-gray-100'; nameColor = 'text-gray-400'; } 
                      ...
                      return (
                        <div key={i} className={`flex items-center justify-between p-2 rounded-xl shadow-sm ${boxBg} ${boxBorder}`}>
                          <div className="flex items-center gap-1.5 overflow-hidden">
                            ...
                            <span className={`text-sm font-bold truncate ${nameColor}`}>{displayName(p)}</span>
                          </div>
                          {game.status === '종료' && game.finalResults && (
                            <div className="flex flex-col items-end pr-1">
                              <span className={`text-sm font-black ${game.finalResults[i].pt > 0 ? 'text-[#2E7D32]' : game.finalResults[i].pt < 0 ? 'text-red-500' : 'text-gray-400'}`}>
              ...
              <div className="bg-[#2E7D32] bg-opacity-5 p-3 rounded-xl border border-green-100 flex justify-between items-center text-sm font-bold shadow-inner">
                {players.map((p, i) => (
                  <div key={i} className={`text-center flex-1 flex flex-col items-center ${i > 0 ? "border-l border-green-200 border-opacity-50" : ""}`}>
                    ...
                    <span className="block text-gray-800">{displayName(p)}</span>
                  </div>
                ))}
              </div>
              {currentGame.status === '종료' && currentGame.finalResults && (
                <div className="bg-[#1e293b] text-white rounded-xl p-4 mt-4 relative shadow-lg animate-in fade-in">
                  {canWrite && <button onClick={handleOpenEndGame} className="absolute top-3 right-3 text-[10px] font-bold bg-gray-700 hover:bg-gray-600 px-2 py-1 rounded flex items-center gap-1 transition-colors"><Edit size={10}/> 수정</button>}
                  <h3 className="font-bold text-base mb-3 flex items-center gap-2"><Trophy size={16} className="text-yellow-400"/> 대국 결과</h3>
                  <div className="space-y-1.5">{players.map((p, i) => (<div key={i} className="flex justify-between items-center py-1.5 border-b border-gray-700 last:border-0 text-sm"><span className="font-bold">{displayName(p)}</span><div className="flex items-center gap-3 text-right"><span className="w-16 font-medium text-gray-300 text-xs">{Number(currentGame.finalResults[i].score).toLocaleString()}</span><span className={`w-12 font-black ${parseFloat(currentGame.finalResults[i].pt) > 0 ? 'text-green-400' : parseFloat(currentGame.finalResults[i].pt) < 0 ? 'text-red-400' : 'text-gray-400'}`}>{parseFloat(currentGame.finalResults[i].pt) > 0 ? '+' : ''}{currentGame.finalResults[i].pt}</span></div></div>))}</div>
                </div>
              )}
              {currentGame.status === '진행중' && canWrite && (
                <button onClick={handleOpenEndGame} className="w-full bg-[#1e293b] text-white font-bold py-3.5 rounded-xl mt-4 flex justify-center items-center gap-2 hover:bg-gray-800 transition-colors shadow-md text-sm"><Flag size={16} /> 대국 종료 및 점수 입력</button>
                      ...
                      {record.type === '화료' ? (
                        <>
                          <div className="flex items-center gap-2 mb-2">
                            <span className={`font-bold text-[11px] ${record.winType === '쯔모' ? 'text-[#2E7D32]' : 'text-orange-500'}`}>{record.winType}</span>
                            <span className="font-bold text-sm text-gray-800">
                              {displayName(record.winner)} 
                              {record.winType === '론' && <span className="text-gray-400 text-xs font-medium mx-1">→ {displayName(record.loser)}</span>}
                            </span>
                            <span className="ml-auto font-black text-[#2E7D32] text-xs">{displayScore}</span>
                          </div>
                        ...
                        <div className="space-y-1.5">
                          ...
                          {!record.abortiveType && (
                            <div className="text-[11px] font-bold text-gray-600 pl-5 space-y-0.5">
                              <p className="text-gray-500">텐파이: <span className="text-gray-800">{record.tenpaiPlayers.length > 0 ? record.tenpaiPlayers.map(displayName).join(', ') : '전원 노텐'}</span></p>
                              {record.nagashiMangan?.length > 0 && (
                                <p className="text-gray-500 mt-1">유국만관: {record.nagashiMangan.map(p => <span key={p} className="bg-red-100 text-red-600 px-1.5 py-0.5 rounded ml-1">{displayName(p)}</span>)}</p>
                              )}
                            </div>
                          )}
                        </div>
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5">
                            <AlertOctagon size={14} className="text-gray-800" />
                            <span className="font-bold text-gray-800 text-sm">{displayName(record.chomboPlayer)} 촌보</span>
                          </div>
                        </div>
                      )}
            ...
            <div className="p-4 pb-10">
              {statsSubTab === '플레이어' && (
                <div className="space-y-4">
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Search size={16} className="text-gray-400" /></div>
                    <input type="text" placeholder="플레이어 검색..." value={statsSearchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-xl text-sm font-medium focus:outline-none focus:border-[#2E7D32]" />
                  </div>
                  ...
                  {filteredPlayerStatsList.length === 0 ? (
                    ...
                    filteredPlayerStatsList.map(stat => (
                      <div key={stat.name} onClick={() => setSelectedStatPlayerName(stat.name)} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden cursor-pointer hover:shadow-md transition-all active:scale-[0.98]">
                        <div className="p-4 border-b border-gray-100 flex justify-between items-center">
                          <div>
                            <h3 className="font-bold text-xl text-gray-800">{displayName(stat.name)}</h3>
                            <span className="text-xs text-gray-500 font-bold mt-1 inline-block">{stat.gamesPlayed}국 </span>
                          </div>
        ...
        {activeNav === '라이벌' && (() => {
          ...
          if (rival1 && rival2 && rivalGames.length > 0) {
            rivalGames.forEach(g => {
              ...
              g.rounds.filter(r => r.isActive !== false).forEach(r => {
                if (r.type === '화료' && r.winType === '론') {
                  const dealerIndex = (r.roundNum - 1) % g.players.length;
                  const isDealer = g.players.indexOf(r.winner) === dealerIndex;
                  const { pureTotal } = getMahjongScore(r.han, r.fu, isDealer, false, 0, g.type === '3인');
          ...
          return (
            <div className="flex-1 flex flex-col bg-[#F5F5DC] p-4 space-y-4">
              ...
              {!rival1 || !rival2 ? (
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                  <div className="bg-[#1e293b] text-white p-4 rounded-2xl shadow-lg flex justify-center items-center gap-6">
                    <div className="text-center flex-1"><span className="block text-2xl font-black">{p1Wins}승</span><span className="text-[10px] text-gray-400">{displayName(rival1)} 우위</span></div>
                    <div className="flex flex-col items-center"><span className="text-xs font-bold text-gray-400 bg-gray-800 px-3 py-1 rounded-full">총 {rivalGames.length}국 동탁</span></div>
                    <div className="text-center flex-1"><span className="block text-2xl font-black">{p2Wins}승</span><span className="text-[10px] text-gray-400">{displayName(rival2)} 우위</span></div>
                  </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div className="flex p-4 items-center justify-between">
                      ...
                      <div className="text-center w-[45%]">
                        <span className="block text-[11px] font-bold text-gray-600 mb-2 bg-gray-100 py-1.5 rounded-lg border border-gray-200">
                          {displayName(rival1)} <span className="text-black-400">→</span> {displayName(rival2)}
                        </span>
                        <span className="text-2xl font-black text-gray-800">{p1RonP2}회</span>
                        <div className="mt-3 space-y-1 bg-gray-50 p-2 rounded-lg border border-gray-100">
                          <div className="text-[10px] text-gray-500 flex justify-between px-1"><span className="font-bold">최고 타점</span><span className="font-black text-[#2E7D32]">{p1RonP2Max > 0 ? p1RonP2Max.toLocaleString() : 0}점</span></div>
                      ...
                      <div className="text-center w-[45%]">
                        <span className="block text-[11px] font-bold text-gray-600 mb-2 bg-gray-100 py-1.5 rounded-lg border border-gray-200">
                          {displayName(rival2)} <span className="text-black-500">→</span> {displayName(rival1)}
                        </span>
                        <span className="text-2xl font-black text-gray-800">{p2RonP1}회</span>
                        <div className="mt-3 space-y-1 bg-gray-50 p-2 rounded-lg border border-gray-100">
                          <div className="text-[10px] text-gray-500 flex justify-between px-1"><span className="font-bold">최고 타점</span><span className="font-black text-blue-600">{p2RonP1Max > 0 ? p2RonP1Max.toLocaleString() : 0}점</span></div>
          ...
          <div className="flex-1 flex flex-col p-4 space-y-4">
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 animate-in fade-in slide-in-from-bottom-2">
              <div className="flex items-center justify-between mb-4 border-b border-gray-100 pb-3">
                ...
                <span className="text-sm font-bold text-gray-400">2026/03/07</span>
              </div>
              <ul className="text-sm font-bold text-gray-700 space-y-2 pl-2 list-disc list-inside">
                <li>독립된 플레이어 데이터베이스를 구축하고 데이터 이전을 완료하였습니다.</li>
                <li>동명이인 발생 시 자동으로 A, B 등 알파벳이 부여되어 구분됩니다. (UI에서는 숨김처리)</li>
                <li>새 대국 추가 시 플레이어 이름 자동완성 드롭다운 기능이 완벽하게 추가되었습니다.</li>
                <li>대국/라운드 삭제 시 복구가 가능하도록 소프트 삭제로 변경되었습니다. (통계 제외)</li>
                <li>기록 추가 모달에서 화료자/방총자 선택이 최상단으로 이동하였습니다.</li>
              </ul>
            </div>
          </div>
        )}
              ...
              {breakdownData.data.length === 0 ? (
                ...
                breakdownData.data.map((item) => {
                  ...
                  return (
                    <div key={item.name} className="flex items-center gap-3">
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-black text-white ${rankColor}`}>{item.rank}</div>
                      <span className="w-20 text-sm font-bold text-gray-700 truncate">{displayName(item.name)}</span>
                      <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                        <div className="h-full bg-[#2E7D32]" style={{ width: `${pct}%` }}></div>
                      </div>
                      <span className="w-8 text-right text-sm font-black text-[#2E7D32]">{item.count}회</span>
      ...
      {selectedStatPlayer && (
        <div className="absolute inset-0 bg-black bg-opacity-70 z-[90] flex flex-col justify-end animate-in fade-in">
          {(() => {
            ...
            const rtAllGames = games.filter(g => g.isActive !== false && g.status === '종료' && g.players.includes(selectedStatPlayer.name));
            let rtTotalRounds = 0, rtTotalWins = 0, rtTotalDealIns = 0, rtTotalWinScore = 0;
            let rtFuroWins = 0, rtRyanmenWins = 0, rtLuckPoints = 0, rtGames4 = 0, rtRentai4 = 0;
            ...
            const handleWinBreakdown = () => {
              const stats = { '쯔모': 0 };
              modalGames.forEach(g => g.rounds.filter(r => r.isActive !== false).forEach(r => {
                if (r.type === '화료' && r.winner === selectedStatPlayer.name) {
                  if (r.winType === '쯔모') stats['쯔모']++;
                  else if (r.loser) stats[r.loser] = (stats[r.loser] || 0) + 1;
                }
              ...
              let filterText = playerStatTab === '전체' ? '전체 마작' : playerStatTab;
              let seasonText = selectedSeason === 'all' ? '전체 시즌' : seasons.find(s => s.id === selectedSeason)?.name || '';
              setBreakdownData({ title: `${displayName(selectedStatPlayer.name)}의 화료 대상 (${seasonText}, ${filterText})`, data: rankedData });
            };
            ...
            const handleDealInBreakdown = () => {
              ...
              let filterText = playerStatTab === '전체' ? '전체 마작' : playerStatTab;
              let seasonText = selectedSeason === 'all' ? '전체 시즌' : seasons.find(s => s.id === selectedSeason)?.name || '';
              setBreakdownData({ title: `${displayName(selectedStatPlayer.name)}의 방총 대상 (${seasonText}, ${filterText})`, data: rankedData });
            };
            ...
            const cumulativeGames = chronologicalGames.map(g => {
              ...
              currentTotalUma += pt;
              return { 
                id: g.id, date: g.date, pt: parseFloat(currentTotalUma.toFixed(1)), gamePt: pt 
              };
            });
            ...
            return (
              <div className="bg-[#F5F5DC] w-full h-[90%] rounded-t-3xl shadow-2xl flex flex-col animate-in slide-in-from-bottom">
                <div className="bg-[#1e293b] rounded-t-3xl p-4 flex justify-between items-center text-white shrink-0">
                  <div>
                    <h2 className="text-xl font-bold flex items-center gap-2"><PieChart size={20}/> {displayName(selectedStatPlayer.name)}</h2>
                    <select value={selectedSeason} onChange={(e) => setSelectedSeason(e.target.value)} className="bg-gray-700 text-white text-[10px] font-bold py-1 px-2 mt-1 rounded appearance-none focus:outline-none">
                      <option value="all">전체 시즌</option>
                      {seasons.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                    </select>
      ...
      {isNewPlayerModalOpen && (
        <div className="absolute inset-0 bg-black bg-opacity-60 z-[80] flex items-center justify-center animate-in fade-in">
          <div className="bg-white w-11/12 max-w-sm rounded-2xl shadow-2xl p-6 relative">
            <button onClick={() => setIsNewPlayerModalOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600"><X size={20}/></button>
            <h2 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2"><UserPlus size={20}/> 신규 플레이어 등록</h2>
            <div className="space-y-4">
              <p className="text-xs font-bold text-gray-500">알파벳을 제외하고 이름만 기입해주세요. (동명이인은 자동으로 A, B, C가 부여됩니다)</p>
              <input type="text" placeholder="이름 (예: 홍길동)" value={newPlayerBaseName} onChange={e => setNewPlayerBaseName(e.target.value)} className="w-full p-3 border border-gray-300 rounded-xl focus:outline-none focus:border-blue-500 font-bold text-sm" />
              <button onClick={handleRegisterPlayer} className="w-full bg-blue-600 text-white font-bold py-3.5 rounded-xl hover:bg-blue-700 active:scale-[0.98]">등록하기</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-gray-100 w-full h-[85%] rounded-t-3xl shadow-2xl flex flex-col animate-in slide-in-from-bottom">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              <div className="bg-red-50 p-4 rounded-xl border border-red-200 mb-4 shadow-sm">
                <h3 className="font-bold text-red-800 text-sm mb-2 flex items-center gap-2"><Database size={16} /> 기존 데이터베이스 통합 마이그레이션</h3>
                <p className="text-xs text-red-600 font-medium mb-3">과거 대국에 기록된 모든 플레이어를 새 테이블(players)로 이동시키고, 이름 뒤에 A를 일괄 부여합니다. 이 작업은 <strong>초기 1회만</strong> 실행하세요.</p>
                <button onClick={handleMigrateDB} className="w-full bg-red-600 text-white font-bold text-sm py-2 rounded-lg hover:bg-red-700 shadow-sm transition-colors">🔥 DB 마이그레이션 실행</button>
              </div>
        ...
        <div className="absolute inset-0 bg-black bg-opacity-60 z-50 flex flex-col justify-end animate-in fade-in">
          <div className="bg-[#F5F5DC] w-full h-[80%] rounded-t-3xl shadow-2xl flex flex-col animate-in slide-in-from-bottom">
            ...
            <div className="p-5 flex-1 overflow-y-auto">
              ...
              <div className="flex items-center gap-3 bg-white p-2 rounded-xl shadow-sm border border-gray-100 mb-2">
                <div className="bg-gray-700 text-white w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm shadow-inner"><CalendarPlus size={14}/></div>
                <input type="date" value={newGameDate} onChange={e => setNewGameDate(e.target.value)} className="flex-1 text-sm font-bold bg-transparent focus:outline-none text-gray-800" />
              </div>
              <div className="text-right mb-5">
                <button onClick={() => setIsNewPlayerModalOpen(true)} className="text-[11px] font-bold text-blue-600 hover:text-blue-800 underline decoration-blue-300 underline-offset-2">
                  처음으로 경기한 플레이어가 있나요? 신규 플레이어 등록
                </button>
              </div>
              ...
              <div className="space-y-3">
                {/* 💡 완벽한 자동완성이 들어간 인풋 렌더링 함수 사용 */}
                {renderPlayerInput('동', 'E', playerE, setPlayerE)}
                {renderPlayerInput('남', 'S', playerS, setPlayerS)}
                {renderPlayerInput('서', 'W', playerW, setPlayerW)}
                {newGameType === '4인' && renderPlayerInput('북', 'N', playerN, setPlayerN)}
              </div>
            ...
            {recordMode === '화료' ? (
              <>
                ...
                <section className="space-y-3 mt-4">
                  <h3 className="font-bold text-base text-gray-800 border-b pb-1">화료 형태</h3>
                  <div className="flex gap-2">
                    <button onClick={() => handleWinTypeChange('쯔모')} className={`flex-1 py-3 rounded-xl font-bold border-2 ${winType === '쯔모' ? 'border-[#2E7D32] bg-[#2E7D32] text-white' : 'bg-white text-gray-400 border-gray-100'}`}>쯔모</button>
                    <button onClick={() => handleWinTypeChange('론')} className={`flex-1 py-3 rounded-xl font-bold border-2 ${winType === '론' ? 'border-[#2E7D32] bg-[#2E7D32] text-white' : 'bg-white text-gray-400 border-gray-100'}`}>론</button>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => setMenzen('멘젠')} className={`flex-1 py-3 rounded-xl font-bold border-2 ${menzen === '멘젠' ? 'border-[#2E7D32] bg-[#2E7D32] text-white' : 'bg-white text-gray-400 border-gray-100'}`}>멘젠</button>
                    <button onClick={() => setMenzen('비멘젠')} className={`flex-1 py-3 rounded-xl font-bold border-2 ${menzen === '비멘젠' ? 'border-[#2E7D32] bg-[#2E7D32] text-white' : 'bg-white text-gray-400 border-gray-100'}`}>비멘젠</button>
                  </div>
                </section>
              ...
              <>
                <section className="space-y-3"><div className="flex justify-between items-end border-b pb-1"><h3 className="font-bold text-base text-gray-800">텐파이 플레이어</h3><p className="text-[10px] text-gray-400">선택 안하면 전원 노텐</p></div><div className="grid grid-cols-2 gap-2">{players.map((player, index) => <button key={`tenpai-${index}`} onClick={() => toggleTenpai(index)} disabled={abortiveType !== null} className={`h-12 rounded-xl font-bold text-sm transition-all border-2 select-none ${tenpaiPlayers.includes(index) ? 'bg-[#2E7D32] border-[#2E7D32] text-white shadow-inner' : 'bg-white border-gray-200 text-gray-800 disabled:opacity-50'}`}>{displayName(player)}</button>)}</div></section>
                <section className="space-y-3"><div className="flex justify-between items-end border-b pb-1"><h3 className="font-bold text-base text-gray-800">유국만관</h3></div><div className="grid grid-cols-2 gap-2">{players.map((player, index) => <button key={`nagashi-${index}`} onClick={() => toggleNagashi(index)} disabled={abortiveType !== null} className={`h-12 rounded-xl font-bold text-sm transition-all border-2 select-none ${nagashiMangan.includes(index) ? 'bg-red-500 border-red-500 text-white shadow-inner' : 'bg-white border-gray-200 text-gray-600 disabled:opacity-50'}`}>{displayName(player)}</button>)}</div></section>
                <section className="space-y-3"><div className="flex justify-between items-end border-b pb-1"><h3 className="font-bold text-base text-gray-800">도중유국 (선택)</h3></div><div className="grid grid-cols-2 gap-2">{abortiveDraws.map(type => <button key={type} onClick={() => toggleAbortive(type)} className={`h-12 rounded-xl text-center font-bold text-sm border-2 transition-colors ${abortiveType === type ? 'bg-gray-700 border-gray-700 text-white shadow-inner' : 'bg-white border-gray-200 text-gray-600 hover:border-gray-300'}`}>{type}</button>)}</div></section>
              </>
            ) : (
              <>
                <section className="space-y-3"><div className="flex justify-between items-end border-b pb-1"><h3 className="font-bold text-base text-red-600">촌보 플레이어 선택</h3></div><div className="grid grid-cols-2 gap-2">{players.map((player, index) => <button key={`chombo-${index}`} onClick={() => setChomboPlayer(index)} className={`h-14 rounded-xl font-bold text-base transition-all border-2 select-none ${chomboPlayer === index ? 'bg-red-500 border-red-500 text-white shadow-inner' : 'bg-white border-gray-200 text-gray-800'}`}>{displayName(player)}</button>)}</div></section>
              </>
            )}
            <section className="pt-4 border-t border-gray-200"><textarea placeholder="해당 국에 대한 메모나 코멘트를 자유롭게 적어주세요. (선택)" value={roundComment} onChange={(e) => setRoundComment(e.target.value)} className="w-full p-3 rounded-xl border border-gray-200 bg-white text-sm focus:outline-none focus:border-[#2E7D32] h-20 resize-none shadow-sm"></textarea></section>
          </div>
      ...
      {isEndGameModalOpen && (
        <div className="absolute inset-0 bg-black bg-opacity-60 z-[60] flex flex-col justify-end animate-in fade-in">
          <div className="bg-[#F5F5DC] w-full h-[70%] rounded-t-3xl shadow-2xl flex flex-col animate-in slide-in-from-bottom">
            ...
            <div className="p-4 flex-1 overflow-y-auto space-y-4">
              <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 p-3 rounded-lg text-xs font-bold shadow-sm">⚠️ 소점 총합은 {players.length === 4 ? '100,000' : '105,000'}점이어야 합니다.<br/>현재 입력 합계: <span className="text-red-500">{finalScores.reduce((sum, f) => sum + (parseInt(f.score) || 0), 0).toLocaleString()}점</span></div>
              <div className="space-y-2">{players.map((p, i) => (<div key={i} className="flex gap-3 items-center bg-white p-3 rounded-xl border border-gray-200 shadow-sm"><span className="w-14 font-bold truncate text-gray-800 text-sm">{displayName(p)}</span><input type="text" placeholder="소점 (예: -500)" value={finalScores[i]?.score ?? ''} onChange={(e) => updateFinalScore(i, e.target.value)} className="flex-1 p-2 bg-gray-50 border border-gray-200 rounded-lg text-right font-bold text-sm focus:outline-none focus:border-[#2E7D32]" /></div>))}</div>
              <p className="text-center text-gray-400 text-[10px] font-bold mt-2">※ PT(우마/오카)는 자동 계산됩니다.</p>
            </div>
            <div className="p-4 bg-white border-t border-gray-200 shadow-md"><button onClick={handleConfirmEndGame} className="w-full bg-[#1e293b] text-white font-bold text-sm py-3.5 rounded-xl hover:bg-gray-800 active:scale-[0.98] transition-all">결과 저장 및 대국 종료</button></div>
          </div>
